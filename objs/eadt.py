		...
		for side in ["LEFT","RIGHT"]:
			...
			if isEADT and isAnkle and isKnee:
				...
				if side == "LEFT":						
					...
					f_angle = abs(180 - angle)

					if angle >= 180:
						m_text = '{0:.1f} - 180 = {1:.1f}'.format(angle, f_angle)
					else:
						m_text = '180 - {0:.1f} = {1:.1f}'.format(angle, f_angle)
				...
				if self.draw_labels:
					...
					self.draw_tools.create_mytext(eadt_p1, m_text, self.tag, x_offset=60, color="blue")
			


				if self.dict["EXCEL"][self.op_type][side]["EADTA"] == None:
					self.dict["EXCEL"][self.op_type][side]["HASDATA"] 	= True
					self.dict["EXCEL"][self.op_type][side]["EADTA"]	 	= '{0:.1f}'.format(f_angle)
					self.controller.save_json()
