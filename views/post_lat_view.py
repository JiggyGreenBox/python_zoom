...
class POST_LAT_View(tk.Frame):
	...
	def update_dict(self, master_dict):
		...
		if self.master_dict["IMAGES"]["POST-LAT"] != None:

			# self.med_image = self.master_dict["IMAGES"]["PRE-LAT"]
			self.med_image = self.controller.working_dir + "/" + self.master_dict["IMAGES"]["POST-LAT"]
	...
	def open_image_loc(self):
		...
		if isinstance(image, str) and image != "":
			...
			self.master_dict["IMAGES"]["POST-LAT"] = rel_path
	...
	def getViewPointSize(self):
		return int(self.master_dict["POINT_SIZES"]["POST-LAT"])
