...
class CanvasImage:
	...
	def __init__(self, placeholder, path):
		...
		self.pilu_image = Image.open(self.path)
		self.pilu_draw = ImageDraw.Draw(self.pilu_image)
	...
	def jiggy(self, event):
		...
		if len(point_list) == 3:
			...
			self.pilu_draw.line([x, y, x1, y1], fill=(255,165,0), width=2)
			self.pilu_draw.line([x1, y1, x2, y2], fill=(255,165,0), width=2)
			...
			print('angle: {}, angle with x-axis: {}'.format(angle1, arc_angle1))

			arc_angle = -1*arc_angle1 + angle2
			angle = -1*arc_angle1


			r = 20

			self.pilu_draw.arc([x1-r, y1-r, x1+r, y1+r], start=arc_angle, end=angle, fill=(255,165,0), width=7)

			filename = "my_angle.jpg"
			self.pilu_image.save(filename)
